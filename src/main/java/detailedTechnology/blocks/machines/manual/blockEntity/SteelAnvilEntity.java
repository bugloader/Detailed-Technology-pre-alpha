...
import detailedTechnology.blocks.machines.manual.screenHandler.AnvilScreenHandler;
...
public class SteelAnvilEntity extends BlockEntity implements ImplementedInventory, NamedScreenHandlerFactory, Tickable {
    ...
    public SteelAnvilEntity(){
        super(Manual.steelAnvilEntity);
        workingTime=0;
        currentCraftingId=-1;
    }
    ...
    public ScreenHandler createMenu(int syncId, PlayerInventory playerInventory, PlayerEntity player) {
        return new AnvilScreenHandler(syncId, playerInventory, this,propertyDelegate);
    }

    public boolean addWorkingTime(String tool,int level,PlayerEntity playerEntity){
        boolean result = false;
        if(currentCraftingId!=-1&&
                tool.equals(AnvilRecipe.TOOL_TYPE.get(currentCraftingId))&&
                Math.min(level,2)>=AnvilRecipe.TOOL_LEVEL.get(currentCraftingId)){
