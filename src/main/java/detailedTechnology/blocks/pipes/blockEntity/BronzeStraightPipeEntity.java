...
import detailedTechnology.blocks.pipes.block.StraightPipe;
...
public class BronzeStraightPipeEntity extends BlockEntity implements Tickable {
    ...
    public BronzeStraightPipeEntity() {
        super(Pipes.bronzeStraightPipeEntity);
        liquidName = "air";
        liquidAmount = 0;
    }
    ...
    private void updateConnection(){
        if(updateTimer==0) {
            int facing = Objects.requireNonNull(world).getBlockState(pos).get(StraightPipe.PIPE_FACING);
            pipeUtilities.setConnection(((StraightPipe) world.getBlockState(pos).getBlock()).getConnected(facing));
        }
