...
import detailedTechnology.blocks.pipes.block.CrossPipe;
...
public class BronzeCrossPipeEntity extends BlockEntity implements Tickable {
    ...
    public BronzeCrossPipeEntity() {
        super(Pipes.bronzeCrossPipeEntity);
        liquidName = "air";
        liquidAmount = 0;
    }
    ...
    private void updateConnection(){
        if(updateTimer==0) {
            int facing = Objects.requireNonNull(world).getBlockState(pos).get(CrossPipe.PIPE_FACING);
            pipeUtilities.setConnection(((CrossPipe) world.getBlockState(pos).getBlock()).getConnected(facing));
        }
