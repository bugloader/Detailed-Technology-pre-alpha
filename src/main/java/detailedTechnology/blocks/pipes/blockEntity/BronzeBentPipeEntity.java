...
import detailedTechnology.blocks.pipes.block.BentPipe;
...
public class BronzeBentPipeEntity extends BlockEntity implements Tickable {
    ...
    public BronzeBentPipeEntity() {
        super(Pipes.bronzeBentPipeEntity);
        liquidName = "air";
        liquidAmount = 0;
    }
    ...
    private void updateConnection(){
        if(updateTimer==0) {
            int facing = Objects.requireNonNull(world).getBlockState(pos).get(BentPipe.PIPE_FACING);
            pipeUtilities.setConnection(((BentPipe) world.getBlockState(pos).getBlock()).getConnected(facing));
        }
