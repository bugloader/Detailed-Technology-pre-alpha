...
import detailedTechnology.blocks.pipes.block.FiveWayPipe;
...
public class BronzeFiveWayPipeEntity extends BlockEntity implements Tickable {
    ...
    public BronzeFiveWayPipeEntity() {
        super(Pipes.bronzeFiveWayPipeEntity);
        liquidName = "air";
        liquidAmount = 0;
    }
    ...
    private void updateConnection(){
        if(updateTimer==0) {
            int facing = Objects.requireNonNull(world).getBlockState(pos).get(FiveWayPipe.PIPE_FACING);
            pipeUtilities.setConnection(((FiveWayPipe) world.getBlockState(pos).getBlock()).getConnected(facing));
        }
