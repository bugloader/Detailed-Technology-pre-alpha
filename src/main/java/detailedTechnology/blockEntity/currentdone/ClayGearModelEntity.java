...
public class ClayGearModelEntity extends BlockEntity implements ImplementedInventory, NamedScreenHandlerFactory, Tickable {
    ...
    public ClayGearModelEntity() {
        super(Machines.clayGearModelEntity);
        liquidName = "air";
        temperature = 20;
    }
    ...
    private void updateLiquid(Inventory inventory) {
        ...
        if (temperature <= solidifyTemp) {
            ...
            switch (liquidName) {
                case "copper":
                    if (itemCount == 0) {
                        inventory.setStack(0, Ores.copperGear.getDefaultStack());
                        liquidName = "air";
                    } else if (itemCount < 64 &&
                            inventory.getStack(0).getName().getString().equals(Ores.copperGear.getName().getString())) {
                ...
                case "tin":
                    if (itemCount == 0) {
                        inventory.setStack(0, Ores.tinGear.getDefaultStack());
                        liquidName = "air";
                    } else if (itemCount < 64 &&
                            inventory.getStack(0).getName().getString().equals(Ores.tinGear.getName().getString())) {
                        inventory.getStack(0).setCount(itemCount + 1);
                        liquidName = "air";
                    }
                    break;
                case "bronze":
                    if (itemCount == 0) {
                        inventory.setStack(0, Ores.bronzeGear.getDefaultStack());
                        liquidName = "air";
                    } else if (itemCount < 64 &&
                            inventory.getStack(0).getName().getString().equals(Ores.bronzeGear.getName().getString())) {
                        inventory.getStack(0).setCount(itemCount + 1);
                        liquidName = "air";
                    }
                    break;
